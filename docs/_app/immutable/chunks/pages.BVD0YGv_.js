...
`,pn=av(),La=Vs(Ms.DIRECTIVES,[]);var Ef;const W_=Ys((Ef=pn.getQueryType())==null?void 0:Ef.getFields());var _f;const J_=Ys((_f=pn.getMutationType())==null?void 0:_f.getFields());var vf;const X_=Ys((vf=pn.getSubscriptionType())==null?void 0:vf.getFields()),ru=bt.keyBy(kp(),r=>r.name.toLocaleLowerCase()),Z_=bt.mapValues(ru,r=>{const n=La.find(t=>(t==null?void 0:t.name)===r.name||(t==null?void 0:t.name)==="*");return n?n.args.some(t=>t==="*")?r.args:n.args.map(t=>r.args.find(s=>s.name===t)).filter(t=>!!t):[]}),K_=Ys(pn.getTypeMap()),ev=j_(pn);function Ys(r){return bt.mapKeys(r||{},(n,t)=>t.toLocaleLowerCase())}function nv(){return bt.size(pn.getTypeMap())<=10}function tv(){return[va("Queries",pn.getQueryType()),va("Mutations",pn.getMutationType()),va("Subscriptions",pn.getSubscriptionType()),uv(),sv()].filter(r=>!!r)}function va(r,n){return iv(r,rv(n))}function rv(r){return bt.sortBy((r==null?void 0:r.getFields())||{},n=>n.name)}function iv(r,n){return n.length===0?null:{type:"menu",title:r,children:n.map(t=>({type:"page",title:t.name,section:r,deprecated:!!t.deprecationReason,href:Ci.joinUrlPaths(Gs(),r.toLocaleLowerCase(),t.name)}))}}function sv(){return nv()?null:{type:"menu",title:"Types",children:bt.sortBy(bt.map(pn.getTypeMap()),n=>n.name).filter(n=>!n.name.startsWith("__")).map(n=>({type:"page",title:n.name,section:"Types",href:Ci.joinUrlPaths(Gs(),"types",n.name)}))}}function _v(){return!!pn.getQueryType()}function vv(r){return W_[r.toLocaleLowerCase()]}function Cv(){return!!pn.getMutationType()}function Av(r){return J_[r.toLocaleLowerCase()]}function Fv(){return!!pn.getSubscriptionType()}function Nv(r){return X_[r.toLocaleLowerCase()]}function bv(r){return K_[r.toLocaleLowerCase()]}function ov(r){return ru[r.toLocaleLowerCase()]}function wv(r){return ov(r.name)!==void 0}function xv(){return bt.size(ru)>0}function Ov(r){return Z_[r.name.toLocaleLowerCase()]||[]}function kv(r){if(r)return ev.getFor(r)}function Lv(r){return bt.flatMap(r.getFields(),n=>({field:n,possibleDescriptions:Op(n,r)}))}function Op(r,n){return r?r.description?[{description:r.description,from:n}]:je(n)?n.getInterfaces().flatMap(t=>Op(t.getFields()[r.name],t)):[]:[]}function kp(){return La.some(r=>(r==null?void 0:r.name)==="*")?pn.getDirectives().filter(r=>!["include","skip","deprecated","specifiedBy"].includes(r.name)):La.filter(r=>!!(r!=null&&r.name)).map(({name:r})=>r?pn.getDirective(r):void 0).filter(r=>!!r)}function av(){return Tf.trim().length===0?M_(JSON.parse(JSON.stringify({__schema:{types:[]}}))):Q_(Tf)}function uv(){const r=kp();return r.length===0?null:{type:"menu",title:"Directives",children:r.map(n=>({type:"page",title:n.name,href:Ci.joinUrlPaths(Gs(),"directives",n.name),section:"Directives"}))}}const Rv=Vs(Ms.APP_TITLE,"GraphQL Documentation"),Lp=cv().concat(tv());pv(Lp);const lv=Object.freeze(Lp),Bv=fv();function cv(){return Vs(Ms.PAGES,hv()).filter(n=>!!n).map(n=>Bp([],n))}function fv(){const r=dv();if(r)return r.href;throw new Error("No custom pages or query available to use as the root application URL. You need to provide at least one custom page or your schema should contain at least one query/mutation/subscription.")}function pv(r){function n(s,o){for(const a of s){if(a.type==="page"){o(a);continue}a.type==="menu"&&n(a.children,o)}}let t;n(r,s=>{t&&(t.next={title:s.title,section:s.section,href:s.href},s.previous={title:t.title,section:t.section,href:t.href}),t=s})}function dv(){return Rp(()=>!0)}function Pv(r){return Rp(n=>n.href.toLocaleLowerCase()===r.toLocaleLowerCase())}function Rp(r){function n(t){for(const s of t){if(s.type==="page"&&r(s))return s;if(s.type==="menu"){const o=n(s.children);if(o)return o}}return null}return n(lv)}function Bp(r,n){if(typeof n.content=="string")return{type:"page",title:n.title,content:n.content,href:Ci.joinUrlPaths(Gs(),...r,Df(n.title))};const t=r.concat([Df(n.title)]);return{type:"menu",title:n.title,children:n.content.map(s=>({...Bp(t,s),section:n.title}))}}function Df(r){return Ci.generatePathSegment(r,new bf)}function hv(){return[{title:"Introduction",content:[{title:"Welcome",content:`
        # Welcome 🎉
        ...
        `.split(`
`).map(r=>r.trim()).join(`
`)}]}]}export{_D as A,nv as B,bv as C,kv as D,Wt as E,et as F,Ge as G,Gn as H,je as I,Sn as J,D as K,Lv as L,hr as M,Br as N,Dv as O,$s as P,Ff as Q,gi as R,Tv as S,bf as T,Ie as U,Nn as V,Rv as W,bt as _,Cv as a,_v as b,vv as c,Fv as d,Nv as e,Pv as f,Av as g,Bv as h,Af as i,Iv as j,xv as k,ov as l,Ov as m,Gs as n,wv as o,lv as p,lt as q,np as r,pn as s,In as t,Ci as u,Ce as v,Ev as w,Vs as x,Ms as y,li as z};
